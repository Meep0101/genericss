...
public class HeatMapVisual : MonoBehaviour
{
    ...
    private void UpdateHeatMapVisual(){
        MeshUtils.CreateEmptyMeshArrays(grid.GetWidth() * grid.GetHeight(), out Vector3[] vertices, out Vector2[] uv, out int[] triangles); // Creation of empty mesh
    ...
    private void UpdateHeatMapVisual(){
        ...
        for(int x = 0; x < grid.GetWidth(); x++){
            for (int y = 0; y < grid.GetHeight(); y++){
                ...
                float gridValueNormalized = gridValue ? 1f : 0f;    //(float)gridValue / Grid.HEAT_MAP_MAX_VALUE;
                Vector2 gridValueUV = new Vector2(gridValueNormalized, 0f);
                MeshUtils.AddToMeshArrays(vertices, uv, triangles, index, grid.GetWorldPosition(x, y) + quadSize * .5f, 0f, quadSize, gridValueUV, gridValueUV);
            }
        }
